/**
 * Created with JetBrains RubyMine.
 * User: fei
 * Date: 13-10-19
 * Time: 下午4:53
 * To change this template use File | Settings | File Templates.
 */
function ActivitySignUpController($scope,$navigate){
    $scope.goto_price_list=function(){
        var current_activity=JSON.parse(localStorage.getItem("current_activity"));
        $navigate.go('/price_list/'+current_activity["name"],'slide');
    }
    $scope.refreshInformation = function () {
        $scope.users=JSON.parse(localStorage.getItem((JSON.parse(localStorage.getItem("current_activity")))["name"]+"users")||"[]");
    }
    $scope.refreshInformation();
    $scope.go_activity_list=function(){
        if(confirm("确认要返回活动列表吗?")){
            $navigate.go('/activity_list/','slide');
        }
    }
    $scope.btn_show=function(){
        var current_activity = JSON.parse(localStorage.getItem("current_activity"))||{};
        var judge_status={
            started:function(){return false},
            un_start:function(){return true},
            end:function(){return false}
        }
        return judge_status[current_activity['status']]();
    }
    $scope.activity_start=function(){
        var current_activity=JSON.parse(localStorage.getItem("current_activity"));
        var activity_array=JSON.parse(localStorage.getItem("activity_array"));
        if(_.find(activity_array,function(activity){return activity.status=="started"})!=undefined){
            alert("Sorry,尚有正在进行到活动.");
            return false;
        }
        if(confirm("确认开始活动吗?")){
            current_activity["status"]="started";
            _.find(activity_array,function(activity){return activity.name==current_activity.name}).status="started";
            localStorage.setItem("activity_array",JSON.stringify(activity_array));
            localStorage.setItem("current_activity",JSON.stringify(current_activity));
        }

    }
    $scope.activity_end=function(){
        if(confirm("确认结束活动吗?")){
            var current_activity=JSON.parse(localStorage.getItem("current_activity"));
            var activity_array=JSON.parse(localStorage.getItem("activity_array"));
            current_activity["status"]="end";
            _.find(activity_array,function(activity){return activity.name==current_activity.name}).status="end";
            localStorage.setItem("activity_array",JSON.stringify(activity_array));
            localStorage.setItem("current_activity",JSON.stringify(current_activity));
            localStorage.setItem(current_activity["name"]+"_bidding_array","[]");
            $navigate.go('/price_list/'+current_activity["name"],'slide');
        }
    }
    var current_activity=JSON.parse(localStorage.getItem("current_activity")||"{}");
    $scope.btn_flag=(current_activity["status"]!="end");
    $scope.judge_user=function(mss_name,mss_phone){
        var activity_array=JSON.parse(localStorage.getItem("activity_array")||"[]");
        _.each(activity_array, function receive_message(activity){
            handle_message(activity,mss_name,mss_phone)
        });
    }
    native_accessor.inject_judge_user($scope.judge_user);
    function handle_message(activity,mss_name,mss_phone){
        var users=JSON.parse(localStorage.getItem(activity["name"]+"users")||"[]");
        if(activity.status=="started"){
            if(_.find(users,function(user){return user.phone==mss_phone})==undefined){
                var mss={"name":mss_name,"phone":mss_phone};
                users.push(mss);
    //                    native_accessor.send_sms(mss_phone,"恭喜!报名成功");
                localStorage.setItem(activity["name"]+"users",JSON.stringify(users));
                $scope.users=JSON.parse(localStorage.getItem((JSON.parse(localStorage.getItem("current_activity")))["name"]+"users")||"[]");
                $scope.$apply($scope.users);
                return true;
            }
    //            native_accessor.send_sms(mss_phone,"您已报名");
            return false;
        }
    //        native_accessor.send_sms(mss_phone,"Sorry,未有活动处于报名状态.");
    }
}